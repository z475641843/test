...
//        printf("blanck_duty=%d\n", blanck_duty);
        ...
        {   
          case NORMAL_FLAG:
            ...
            if(duty < -RIGHT_MAX)
              duty = -RIGHT_MAX;
            ftm_pwm_duty(FTM1, FTM_CH0, MID - duty);
...
//            break;
        ...
        case BARRIER_FLAG:
            barrier_flag = 1;
            barrier_counter = 0;
            if(barrier_left_black_num > barrier_right_black_num)
              left_barrier = 1;
            else if(barrier_left_black_num < barrier_right_black_num)
              right_barrier = 1;
//            all_error = 0;
            break;
...
//            right_island_see_flag = 0;
            ftm_pwm_duty(FTM1, FTM_CH0, MID + RIGHT_MAX * 5 / 6);       //右转进右环岛
            all_error = RIGHT_MAX * 5 / 6 / K;
//            all_error_old = all_error;
            break;
            
          case RIGHT_ISLAND_OUT_FLAG:
//            stop_flag = 1;
            ftm_pwm_duty(FTM1, FTM_CH0, MID + RIGHT_MAX);      //右转出右环岛
            all_error = RIGHT_MAX / K;
//            all_error_old = all_error;
            break;
            
          case LEFT_ISLAND_IN_FLAG:
//            stop_flag = 1;
            ftm_pwm_duty(FTM1, FTM_CH0, MID - LEFT_MAX * 5 / 6);          //左转进左环岛
            all_error = LEFT_MAX * 5 / 6 / K;
//            all_error_old = all_error;
          break;
          
          case LEFT_ISLAND_OUT_FLAG:
//            stop_flag = 1;
            ftm_pwm_duty(FTM1, FTM_CH0, MID - LEFT_MAX);         //左转出右环岛
            all_error = LEFT_MAX / K;
//            all_error_old = all_error;
          break;
      ...
      if(barrier_flag == 1)          //障碍处理
      {
        if(barrier_counter < 4 && left_barrier == 1)
        {
          ftm_pwm_duty(FTM1, FTM_CH0, MID + RIGHT_MAX * 2 / 3);
          all_error = RIGHT_MAX / K;
        }
        else if(barrier_counter < 4 && right_barrier == 1)
        {
          ftm_pwm_duty(FTM1, FTM_CH0, MID - LEFT_MAX * 2 / 3);
          all_error = LEFT_MAX / K;
        }
//        if(barrier_counter >= 5 && barrier_counter <= 10 && left_barrier == 1)
//          ftm_pwm_duty(FTM1, FTM_CH0, MID + LEFT_MAX * 2 / 3);
//        else if(barrier_counter >= 5 && barrier_counter <= 10 && right_barrier == 1)
//          ftm_pwm_duty(FTM1, FTM_CH0, MID + RIGHT_MAX * 2 / 3);
//        
...
//          left_barrier = 0;
//          right_barrier = 0;
//          ftm_pwm_duty(FTM1, FTM_CH0, MID);
//        }
        if(barrier_counter == 4)
        {
          barrier_flag = 0;
//          forbid_flag = 0;
          left_barrier = 0;
...
//  angle5 = 0.0;
  ...
  angle = (angle4 * 2.55 + angle3 * 1.33 + angle2 * 0.75 + angle1 * 0.1) / (2.55 + 1.33 + 0.75 + 0.1);
  all_error = (error4 * 2.55 + error3 * 1.33 + error2 * 0.75 + error1 * 0.1) / (2.55 + 1.33 + 0.75 + 0.1);
...
void scan()
{
  ...
  uint8 right_island_point[CAMERA_H],right_island_num,right_island_white_num1,right_island_black_num1,right_island_white_num2,half_row_white,half_col_white,add_num;
  ...
  uint8 right_island_out_black,right_island_out_white,right_island_see_black,right_white_to_black1,right_island_out_white1;
  uint16 black_num;
  uint8 col_white1,right_island_in_white_num,right_island_in_white_line,right_white_line_num,right_island_black_counter[CAMERA_H],right_island_one_black,right_island_two_black,right_island_55_black;
  uint8 right_island_left_most_black[CAMERA_H],right_island_three_black;
  uint8 white_2_black_num,barrier_num,barrier_white_point,barrier_white_line;
  col_white = 0;
  row_white1 = 0;
  row_white2 = 0;
  row_black1 = 0;
  ...
  for(i=0;i<CAMERA_H;i++)      //行扫
  {
    ...
    right_island_black_num1 = 0;
    white_num1 = 0;
    black_num1 = 0;
    right_island_black_counter[i] = 0;
    right_island_left_most_black[i] = 0;
    white_2_black_num = 0;
    ...
    for(j=0;j<CAMERA_W;j++)
    {
      ...
      if( j >= 70 && j <= 79 && i > 0  && center[i][j] == 0xff && center[i-1][j] == 0x00 )
        right_white_to_black += 1;
      
      if( j >= 17 && j <= 22 && i < 59 && center[i][j] == 0xff && center[i+1][j] == 0x00 )
        right_white_to_black1 += 1;
      
      if( j >= 69 && j <= 78 && ( i < CAMERA_H - 1 ) && center[i][j] == 0x00 && center[i+1][j] == 0xff )
      ...
      {
        if(i >= 45 && i <= 49 && j < CAMERA_W - 1)
          white_2_black_num += 1;
      }
    }
    if(right_island_black_counter[i] == 1 && right_island_left_most_black[i] == 1)
      right_island_one_black = 1;
    
    if(right_island_black_counter[i] == 2 && right_island_left_most_black[i] == 1)
      right_island_two_black = 1;
    
    if(right_island_black_counter[i] == 3 && right_island_left_most_black[i] == 1)
      right_island_three_black = 1;
    ...
    if(right_island_point[i] < right_island_point[i-1] && i >= 24 && i <= 46)  //右环岛黑点递增
      right_island_num += 1;
    
    if( right_black_add_point[i] < right_black_add_point[i-1] && i > 0)  //左边黑点递增
      add_num += 1;
...
//    printf("circle_num=%d,i=%d\n", circle_num, i);
    ...
    if(white_num1 >= 70 && white_num1 < 80 &&  i >= 41 && i <= 59)
      right_island_out_white1 += 1;
    
    if(right_island_in_white_num >= 40)
      right_island_in_white_line += 1;

  }
  right_len_max = right_island_point[24];
...
//    printf("right_island_num=%d\n", right_island_num);
  if(0)
  {
    ...
    printf("downmost_white_num=%d\n", downmost_white_num);
    printf("right_island_row_black1=%d\n", right_island_row_black1);
    printf("rightmost_black_5=%d\n", rightmost_black_5);
    printf("right_white_to_black1=%d\n", right_white_to_black1);
  }
//  printf("right_island_out_white=%d\n", right_island_out_white);
//  printf("right_island_out_white1=%d\n", right_island_out_white1);
//  printf("add_num=%d\n", add_num);
//  printf("rightmost_black_5=%d\n", rightmost_black_5);
//  printf("col_white1=%d\n", col_white1);
//  printf("downmost_white_num=%d\n", downmost_white_num);
//  printf("in_rightmost_white_num=%d\n", in_rightmost_white_num);
//  printf("in_leftmost_black_num=%d\n", in_leftmost_black_num);
...
//  printf("right_island_out_white=%d\n", right_island_out_white);
//  printf("right_island_out_black=%d\n", right_island_out_black);
//  printf("col_white1=%d\n", col_white1);
  
//  printf("right_island_out_black_num=%d\n", right_island_out_black_num);
//  printf("half_row_white=%d\n", half_row_white);
//  printf("half_col_white=%d\n", half_col_white);
//  printf("right_island_num=%d\n", right_island_num);   
//  printf("right_island_see_black=%d\n", right_island_see_black); 
//  printf("rightmost_black_5=%d\n", rightmost_black_5);
//  printf("right_island_out_white=%d\n", right_island_out_white);
//  printf("right_island_out_white1=%d\n", right_island_out_white1);
//  printf("add_num=%d\n", add_num);
//  if(half_row_white >= 30 && half_row_white <= 35 && half_col_white >= 10 && half_col_white <= 20 && right_island_num >= 7 && right_island_num <= 15 )
...
//    flag = CIRCLE_FLAG;
  if(start_flag == 1 && start_new == 1 && start_old == 1)
    flag = STOP_FLAG;
//  else if(right_island_see_flag == 1 && right_island_in_flag == 1 && right_island_out_white >= 20 && right_island_out_black >= 5 && col_white1 == 0)  //right_island_out_black >= 17
//  {
...
////    right_island_in_flag = 1;
//  }
//  else if(right_island_see_flag == 1 && ( right_island_out_white >= 8 || right_island_out_white1 >= 4  ) && right_island_out_black <= 2 ) //ol_white1 == 0   && && add_num >= 10 && rightmost_black_5 <= 4
//  {
//    flag = RIGHT_ISLAND_IN_FLAG;
//    right_island_in_flag = 1;
//  }
    ...
    flag = BLANCK_FLAG;
  else if(barrier_num >= 3 && barrier_white_line >= 8 && barrier_white_line <= 10)      
    flag = BARRIER_FLAG;
  else
    flag = NORMAL_FLAG;
}
