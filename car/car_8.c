...
#define MID             5050    //舵机中值占空比3550         300HZ 5950  180HZ 3550
#define RIGHT_MAX       1000     //舵机相对中值向右转最大值         1650        1000    
#define LEFT_MAX        1000     //舵机相对中值向左转最大值         2100        1250   
       ...
       if(1)
       {
        ...
        switch(flag)
        {
          ...
          case RIGHT_ISLAND_IN_FLAG:
            ftm_pwm_duty(FTM1, FTM_CH0, MID + RIGHT_MAX * 6 / 7 );
            ftm_pwm_duty(FTM0, FTM_CH0, (int)( val * 1.2 ) );
            break;
            
          case LEFT_ISLAND_IN_FLAG:
            ftm_pwm_duty(FTM1, FTM_CH0, MID - LEFT_MAX * 6 / 7 );
            ftm_pwm_duty(FTM0, FTM_CH0, (int)( val * 1.2 ) );
            break;
            
          case RIGHT_ISLAND_OUT_FLAG:
            ftm_pwm_duty(FTM1, FTM_CH0, MID + RIGHT_MAX * 3 / 4);
            ftm_pwm_duty(FTM0, FTM_CH0, (int)( val * 0.9 ));
            DELAY_MS(220);        
            break;
          
          case LEFT_ISLAND_OUT_FLAG: 
            ftm_pwm_duty(FTM1, FTM_CH0, MID - LEFT_MAX * 3 / 4);
            ftm_pwm_duty(FTM0, FTM_CH0, (int)( val * 0.9 ));
            DELAY_MS(220);
            break;
...
{  
    ...
    uint8 left_in1,right_in1,left_down2,right_down2;
    error1 = 0;
    error2 = 0;
    error3 = 0;
    sum_y1 = 0;
      ...
      if( center[i-1][j] == 0xff && center[i-1][j-1] == 0x00 )
      {
        ...
        if( i >= 15 )
        {
          ...
          if( Left_Black - Left_Black_old >= 3 && i < CAMERA_H )
            left_in1 += 1;
          if( Left_Black != 0 && ( Left_Black_old == 0 || Left_Black_old == 1 ) && left_down == 1 && i < CAMERA_H )
            left_up += 1;
          if( Left_Black - Left_Black_old >= 3 && i < CAMERA_H )
            left_3 += 1;
          if(Left_Black_old - Left_Black >= 3)
  ...
  /********** 看见环岛识别 **********/
  if( ( right_up >= 1 && right_down == 1 && right_see == 1 && leftmost_white <= 20 && white_line >= 5 && left_up == 0 && left_down == 0 && left_see == 0 && left_in1 == 0 && left_down2 == 0)
     || ( right_up1 >= 1 && right_down1 >= 2 && left_white_num == 0 && white_line >= 4 && right_white_num >= 30 ) )  // && left_down2 == 0 
    right_island_see_flag = 1;
  if( ( left_up >= 1 && left_down == 1 && left_see == 1 && rightmost_white <= 20 && white_line >= 5 && right_up == 0 && right_down == 0 && right_see == 0 && right_in1 == 0 && right_down2 == 0 )
     || ( left_up1 >= 1 && left_down1 >= 2 && right_white_num == 0 && white_line >= 4 && left_white_num >= 30 ) )  //&& right_down2 == 0 
